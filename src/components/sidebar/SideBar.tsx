...
export default function SideBar({ onNavigate }: SideBarProps) {
  ...
  useEffect(() => {
    const refresh = () =>
      api
        .get("/support-chats/conversations")
        .then((response) =>
          setChatUnread(response.data.unreadCount || 0)
        )
        .catch(() => undefined);
    ...
    return () => {
      socket?.off("support_conversation_updated", refresh);
    };
  }, [socket]);
  ...
  const menuItems = useMemo(() => {
    ...
    return items;
  }, [isSuperAdmin, isAdmin, chatUnread]);
      ...
      <div className="border-t border-slate-100 p-4">
        <div className="mb-2 flex items-center gap-3 rounded-xl bg-slate-50 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[10px] font-medium uppercase text-emerald-600">
